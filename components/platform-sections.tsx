...
export function PlatformSections() {
  ...
  return <>
    <section id="cadastro" className="relative overflow-hidden bg-[#07111c] px-5 py-24 text-white sm:px-8">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(34,211,238,.1),transparent_25%),radial-gradient(circle_at_85%_80%,rgba(168,85,247,.1),transparent_30%)]" />
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div><span className="text-sm font-semibold uppercase tracking-[.2em] text-cyan-300">Seu próximo passo</span><h2 className="mt-4 max-w-2xl text-4xl font-semibold tracking-tight sm:text-5xl">Cadastre seu negócio. Entre no mapa de oportunidades.</h2><p className="mt-4 max-w-xl leading-7 text-slate-400">Conte-nos onde sua empresa está hoje e o que você busca. Nós transformamos essas respostas em inteligência para conectar a rede.</p></div>
          <div className="flex rounded-xl border border-white/15 bg-white/5 p-1"><button onClick={()=>setView('form')} className={`rounded-lg px-5 py-2 text-sm ${view==='form'?'bg-cyan-300 text-[#061018]':'text-white/70'}`}>Fazer cadastro</button><button onClick={()=>setView('dashboard')} className={`rounded-lg px-5 py-2 text-sm ${view==='dashboard'?'bg-cyan-300 text-[#061018]':'text-white/70'}`}>Ver indicadores</button></div>
        </div>

        {view==='form' && <div className="grid gap-8 lg:grid-cols-[.36fr_.64fr]">
          <aside className="rounded-3xl border border-cyan-300/15 bg-cyan-300/[.045] p-7"><p className="text-sm text-white/55">Seu progresso</p><div className="mt-6 space-y-5">{['Você e sua empresa','Momento do negócio','Conexões de valor'].map((label,i)=><div key={label} className="flex items-center gap-3"><span className={`grid size-9 place-items-center rounded-full border ${step>i+1?'border-cyan-300 bg-cyan-300 text-[#061018]':step===i+1?'border-white bg-white text-[#061018]':'border-white/20 text-white/40'}`}>{step>i+1?<Check className="size-4"/>:i+1}</span><span className={step===i+1?'text-white':'text-white/50'}>{label}</span></div>)}</div><p className="mt-10 border-t border-white/10 pt-6 text-sm leading-6 text-white/55">Leva cerca de 3 minutos. Seus dados ajudam a revelar onde estão as oportunidades da rede.</p></aside>
          <div className="rounded-3xl bg-white p-6 text-[#07111c] shadow-[0_35px_100px_rgba(0,0,0,.3)] sm:p-9">
            {status==='success' ? <div className="grid min-h-[440px] place-items-center text-center"><div><span className="mx-auto grid size-16 place-items-center rounded-2xl bg-[#07111c] text-cyan-300"><Check /></span><h3 className="mt-6 text-3xl font-semibold">Cadastro concluído.</h3><p className="mx-auto mt-3 max-w-md text-slate-600">Seu negócio já faz parte do mapa. A partir daqui, a rede poderá aproximar você de oportunidades, conteúdos e conexões mais relevantes.</p><Button className="mt-7 rounded-xl px-6" onClick={()=>{setStatus('idle');setStep(1)}}>Novo cadastro</Button></div></div> :
            <form onSubmit={submit}>
              <div className="mb-7 flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-700">Etapa {step} de 3</p><h3 className="mt-2 text-2xl font-semibold">{step===1?'Quem é você e sua empresa?':step===2?'Qual é o momento do seu negócio?':'Que conexão faria diferença agora?'}</h3></div><span className="text-sm text-slate-500">{Math.round(step/3*100)}%</span></div>
              ...
              {step===3 && <div className="grid gap-5"><SelectField label="O que você procura nesta rede?" name="seeks" value={data.seeks} options={['Parcerias','Clientes','Fornecedores','Mentoria','Investidores','Tecnologia','Internacionalização','Capacitação','Networking','Impacto social']} onChange={change}/><div className="space-y-2"><Label htmlFor="offers">O que você pode oferecer à rede?</Label><Textarea id="offers" value={data.offers} onChange={e=>change('offers',e.target.value)} placeholder="Conte brevemente sobre sua experiência, conexões ou recursos..." required className="min-h-32"/></div><p className="rounded-2xl bg-cyan-50 p-4 text-sm leading-6 text-slate-600">Ao concluir, você autoriza o uso destes dados para organização da rede e geração de indicadores agregados. Seus dados de contato não aparecem no painel público.</p></div>}
              {status==='error'&&<p className="mt-5 text-sm text-red-700">{message}</p>}
              <div className="mt-8 flex items-center justify-between border-t border-slate-200 pt-6">{step>1?<Button type="button" variant="ghost" onClick={()=>setStep(step-1)}><ChevronLeft/> Voltar</Button>:<span/>}{step<3?<Button type="button" className="h-11 rounded-xl px-6" onClick={()=>setStep(step+1)}>Continuar <ArrowRight/></Button>:<Button type="submit" className="h-11 rounded-xl px-6" disabled={status==='loading'}>{status==='loading'?<Loader2 className="animate-spin"/>:<Network/>} Concluir cadastro</Button>}</div>
            </form>}
          </div>
        </div>}

        {view==='dashboard' && <div className="rounded-[2rem] border border-white/10 bg-[#0b1724] p-5 text-white shadow-2xl sm:p-8"><div className="flex flex-col justify-between gap-4 border-b border-white/10 pb-6 sm:flex-row sm:items-center"><div><p className="text-sm text-slate-400">Visão geral do ecossistema</p><h3 className="mt-1 text-2xl font-semibold">Painel Brasil</h3></div><span className="flex items-center gap-2 text-sm text-slate-400"><span className="size-2 rounded-full bg-emerald-400"/> Atualizado em tempo real</span></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[[displayedTotal,'Empreendedores',Users2],[stats.byState.length||18,'Estados',MapPin],[stats.bySegment.length||42,'Segmentos',BarChart3],['31%','Buscam IA',Globe2]].map(([value,label,Icon])=><article key={String(label)} className="rounded-2xl border border-white/10 bg-white/[.04] p-5"><Icon className="size-5 text-cyan-300"/><strong className="mt-6 block text-3xl">{value}</strong><span className="mt-1 block text-sm text-slate-400">{String(label)}</span></article>)}</div><div className="mt-5 grid gap-5 text-[#07111c] lg:grid-cols-2"><Chart title="Presença por estado" data={demoStates}/><Chart title="Segmentos em destaque" data={demoSegments}/></div><p className="mt-5 text-center text-xs text-slate-500">Enquanto a rede está em demonstração, o painel usa dados ilustrativos. Cadastros reais passam a alimentar os indicadores automaticamente.</p></div>}
      </div>
    </section>
    <footer id="visao" className="border-t border-white/10 bg-[#04080e] px-5 py-10 text-white/65 sm:px-8"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-5 sm:flex-row sm:items-center"><div className="flex items-center gap-3 text-white"><Network className="text-cyan-300"/><strong className="tracking-wider">EMPREENDEDORES DO REINO</strong></div><p className="text-sm">Negócios conectados ao futuro. Impacto que transforma o presente.</p></div></footer>
  </>;
}
